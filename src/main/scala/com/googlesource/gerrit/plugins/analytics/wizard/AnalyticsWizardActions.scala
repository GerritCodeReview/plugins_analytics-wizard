...
class PutAnalyticsStack @Inject()(@PluginData val dataPath: Path)
  ...
  override def apply(resource: ProjectResource,
    ...
    val projectName = resource.getControl.getProject.getName
    AnalyticsWizardActions
      .TryURLEncoder(projectName)
      .fold(throw new RestApiException("Unsupported Encoding Exception")) {
        name =>
          AnalyticDashboardSetup(
            projectName,
            dataPath.resolve(s"docker-compose.${name}.yaml"))
            .createDashboardSetupFile()
          Response.created(s"Dashboard configuration created for $name!")
      }
  }
}
      ...
      .fold(throw new RestApiException("Unsupported Encoding Exception")) {
        name =>
          val pb = new ProcessBuilder(
            "docker-compose",
            "-f",
            s"${dataPath.toFile.getAbsolutePath}/docker-compose.${name}.yaml",
...
object AnalyticsWizardActions {
  ...
  def TryURLEncoder(name: String): Option[String] =
    Try { URLEncoder.encode(name, "UTF-8") }.toOption
}
