...
class PutAnalyticsStack @Inject()(@PluginData val dataPath: Path)
  ...
  override def apply(resource: ProjectResource,
    ...
    val projectName = resource.getControl.getProject.getName
    val encodedName = AnalyticsWizardActions.TryURLEncoder(projectName)
    AnalyticDashboardSetup(
      projectName,
...
object AnalyticsWizardActions {
  // URLEncoder could potentially throw UnsupportedEncodingException, but not in a practical case,
  // hence we can just "swallow" the exception
  def TryURLEncoder(name: String): String =
    Try { URLEncoder.encode(name, "UTF-8") }.get
}
