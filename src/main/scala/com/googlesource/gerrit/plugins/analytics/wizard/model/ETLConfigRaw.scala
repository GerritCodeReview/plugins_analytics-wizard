...
object ETLConfigRaw {
  ...
  private def validateUrl(value: String): Either[UrlValidationError, URL] = {
    val maybeUrl = Try(new URL(value)).toOption
    Either.cond(maybeUrl.isDefined, maybeUrl.get, UrlValidationError(value))
  }
}
...
case class UrlValidationError(value: String) extends ETLConfigValidationError {
  val parameter = "url"
}
