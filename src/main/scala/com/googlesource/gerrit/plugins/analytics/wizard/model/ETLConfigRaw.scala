...
object ETLConfigRaw {
  def apply(aggregate: String,
    ...
    for {
      s <- validateLocalDate(since).right
      u <- validateLocalDate(until).right
    } yield
  ...
  private def validateLocalDate(input: String): Either[ETLConfigValidationError, LocalDate] = {
    val formatter      = DateTimeFormatter.ofPattern("yyyy-MM-dd")
    val maybeLocalDate = Try(LocalDate.parse(input, formatter)).toOption
    Either.cond(maybeLocalDate.isDefined,
                maybeLocalDate.get,
                LocalDateValidationError("since", input))
  }
}
